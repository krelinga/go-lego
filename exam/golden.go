...
// Use GoldenHere to create one at a call site.
type Golden struct {
	loc  Loc
	text string
}
...
// TODO is a placeholder value for golden data that has not yet been generated. Its value is "\n"
...
// where GoldenHere is called. text must begin with a newline; by convention the closing backtick of
// the raw string literal appears on its own line so that the content is unambiguous. As a
// convenience, exam.TODO may be passed as a placeholder until real golden data is available.
...
// StringIndentLines is a FmtFunc that inserts a tab after every newline in val's string
// representation, making multi-line values readable in failure output. It returns an error if val
// is not of kind string.
...
// GoldenEqual asserts that actual matches the expected text stored in expected. actual is prepended
// with a newline before comparison so that multi-line values align naturally with the raw string
...
// Instead of using this flag directly, prefer running tests with the update-goldens tool, which
// sets the flag and applies the recorded diffs after the test run completes.
