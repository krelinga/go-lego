	...
	for _, c := range cases {
		exam.Run(t, c.Name, c.Loc, func(t *testing.T) {
			...
			for _, e := range c.Entries {
				exam.Must(t, exam.Nil(golden.WriteEntry(path, e)))
			}

			got, err := golden.ReadEntries(path)
...
func TestWriteGoldenEntry_MagicNumberWrittenOnce(t *testing.T) {
	path := filepath.Join(t.TempDir(), "goldens.bin")
	entry := golden.Entry{Path: "/workspace/foo_test.go", Line: 1, Text: "\ntext\n"}

	exam.Must(t, exam.Nil(golden.WriteEntry(path, entry)))
	exam.Must(t, exam.Nil(golden.WriteEntry(path, entry)))
...
func TestReadGoldenEntries_TruncatedEntry(t *testing.T) {
	...
	_, err = golden.ReadEntries(path2)
	exam.Try(t, exam.NotNil(err))
}
...
func TestWriteGoldenEntry_AppendsToExistingFile(t *testing.T) {
	path := filepath.Join(t.TempDir(), "goldens.bin")
	e1 := golden.Entry{Path: "/workspace/a_test.go", Line: 1, Text: "\nfirst\n"}
	e2 := golden.Entry{Path: "/workspace/b_test.go", Line: 2, Text: "\nsecond\n"}

	exam.Must(t, exam.Nil(golden.WriteEntry(path, e1)))

	size1, err := os.Stat(path)
	exam.Must(t, exam.Nil(err))

	exam.Must(t, exam.Nil(golden.WriteEntry(path, e2)))
